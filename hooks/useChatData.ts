...
export const useChatData = (
    ...
    // --- REALTIME HANDLER WITH BATCHING ---
        ...
        setChats(prevChats => {
            ...
            updates.forEach(payload => {
                if (payload.eventType === 'INSERT' && payload.table === 'messages') {
                    ...
                    const isMyGroup = newMsg.group_id && updatedChats.some(c => c.id === newMsg.group_id);
                    ...
                    updatedChats = updatedChats.map(c => ({ 
                        ...c, 
                        messages: c.messages.filter(m => m.id !== payload.old.id) 
                    }));
                }
    ...
    const handleRealtimePayload = useCallback((payload: any) => {
        ...
        // Мгновенное обновление без батчинга для быстрого отображения
        flushUpdates();
        
        // Play notification for new messages
        const newMessages = pendingUpdates.current.filter(
            p => p.eventType === 'INSERT' && p.table === 'messages' && p.new.sender_id !== userProfile.id
        );
        
        if (newMessages.length > 0) {
            const lastMsg = newMessages[newMessages.length - 1].new;
            const chat = chatsRef.current.find(c => 
                c.id === (lastMsg.group_id || lastMsg.sender_id)
            );
            
            if (!chat || !chat.isMuted) {
                playNotificationSound();
                const senderName = chat ? chat.user.name : 'Новое сообщение';
                const notificationText = lastMsg.type === 'image' ? 'Фото' : 
                                       lastMsg.type === 'audio' ? 'Голосовое' : 
    ...
    }, [userProfile.id, flushUpdates, playNotificationSound, showToast, sendBrowserNotification]);

    // --- ACTIONS ---
    const sendMessage = async (chatId: string, text: string, type: MessageType = 'text', mediaUrl?: string, duration?: string, fileName?: string, fileSize?: string, replyToId?: string, forwardedFrom?: string, forwardedFromName?: string) => {
        ...
        const payload: any = { 
            ...
            forwarded_from_name: forwardedFromName || null,
    ...
    const handleMarkAsRead = async (chatId: string) => {
        ...
        console.log('Marking as read:', chatId, 'isGroup:', isGroup);
        
        // Сначала обновляем базу данных
        if (isGroup) {
            const { error } = await supabase.from('group_members').update({ last_read_at: new Date().toISOString() }).eq('group_id', chatId).eq('user_id', userProfile.id);
            console.log('Group mark as read result:', error ? 'ERROR' : 'SUCCESS');
        } else {
            const { error, count } = await supabase.from('messages').update({ is_read: true }).eq('receiver_id', userProfile.id).eq('sender_id', chatId).eq('is_read', false);
            console.log('DM mark as read result:', error ? 'ERROR' : 'SUCCESS', 'updated:', count);
        }
        
        // Потом обновляем локальное состояние
        setChats(prev => prev.map(c => {
