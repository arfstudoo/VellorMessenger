...
const App: React.FC = () => {
  ...
  const [newDeviceAlert, setNewDeviceAlert] = useState<{ deviceName: string; deviceId: string } | null>(null);
  ...
  // Device logout listener
  useEffect(() => {
      if (appState !== 'app' || !userProfile.id) return;
      const channel = supabase.channel('device_logout');
      channel.on('broadcast', { event: 'force_logout' }, ({ payload }) => {
          if (payload.user_id === userProfile.id) {
              ...
              if (currentDeviceId === payload.device_id) {
                  // Это наше устройство - немедленно выходим
                  showToast('сеанс завершен', 'warning');
  ...
  useEffect(() => {
      ...
      const checkDeviceExists = async () => {
          ...
          try {
              ...
              if (!deviceExists) {
                  // Устройство было удалено - выходим
                  showToast('сеанс завершен', 'warning');
      ...
      // Первая проверка через 15 секунд (даем время на создание устройства)
      // Затем проверяем каждые 10 секунд
      const firstCheck = setTimeout(checkDeviceExists, 15000);
      const interval = setInterval(checkDeviceExists, 10000);
  ...
  // New device login listener
  useEffect(() => {
      if (appState !== 'app' || !userProfile.id) return;
      
      const channel = supabase
          .channel(`new_device_alert_${userProfile.id}`)
          .on('postgres_changes',
              { event: 'INSERT', schema: 'public', table: 'user_devices', filter: `user_id=eq.${userProfile.id}` },
              (payload) => {
                  console.log('New device detected:', payload);
                  const newDevice = payload.new as any;
                  const currentDeviceId = localStorage.getItem('vellor_device_id');
                  
                  // Проверяем что это не наше устройство
                  if (newDevice.device_id !== currentDeviceId) {
                      console.log('Showing alert for new device:', newDevice.device_name);
                      setNewDeviceAlert({
                      ...
                      console.log('Ignoring own device');
                  }
              }
          )
          .subscribe();
  ...
  useEffect(() => {
    const checkSession = async () => {
      try {
        ...
        if (session) {
          // Проверяем существует ли устройство в базе
          const deviceId = localStorage.getItem('vellor_device_id');
          if (deviceId) {
            const { data: deviceExists } = await supabase
              .from('user_devices')
          ...
          if (profile) {
             ...
             // Track device
             await trackDevice(session.user.id);
  ...
  const trackDevice = async (userId: string) => {
      try {
          ...
          let deviceId = localStorage.getItem('vellor_device_id');
          if (!deviceId) {
              deviceId = `${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
              localStorage.setItem('vellor_device_id', deviceId);
          }
          
          console.log('Tracking device:', deviceId);
          
          // Помечаем все ДРУГИЕ устройства как не текущие
          await supabase
              .from('user_devices')
          ...
          // Используем UPSERT для вставки/обновления устройства
  ...
  const getDeviceInfo = () => {
      ...
      return {
          ...
          deviceType,
          userAgent: ua,
          ipAddress: null, // Can be fetched from backend
          location: null   // Can be fetched from IP geolocation service
      };
  };
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black overflow-hidden bg-black">
      ...
      <AnimatePresence>
          {newDeviceAlert && (
              <MDiv initial={{ y: -100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -100, opacity: 0 }} className="fixed top-24 left-0 right-0 z-[99998] flex justify-center p-4 pointer-events-none">
                  <div className="bg-black/80 backdrop-blur-2xl border border-yellow-500/50 shadow-[0_0_50px_rgba(234,179,8,0.3)] rounded-2xl p-6 max-w-md w-full pointer-events-auto">
                      <div className="flex items-start gap-4 mb-4">
                          <div className="p-3 bg-yellow-500/20 rounded-full shadow-lg shrink-0">
                              <ShieldAlert size={24} className="text-yellow-500"/>
                          </div>
                          <div className="flex-1">
                              <h3 className="text-sm font-black uppercase text-yellow-500 tracking-widest mb-1">Новый вход в аккаунт</h3>
                              <p className="text-white/90 text-sm font-medium leading-relaxed">
                                  Обнаружен вход с устройства: <span className="text-yellow-400 font-bold">{newDeviceAlert.deviceName}</span>
                              </p>
                          </div>
                      </div>
                      <div className="flex gap-2">
                          <button 
                              ...
                              className="flex-1 px-4 py-2.5 bg-green-500/20 border border-green-500/30 rounded-xl text-green-400 text-xs font-bold uppercase tracking-wider hover:bg-green-500/30 transition-all active:scale-95"
                          >
                              Это я
                          </button>
                          <button 
                              onClick={async () => {
                                  if (newDeviceAlert) {
                                      await supabase.from('user_devices').delete().eq('device_id', newDeviceAlert.deviceId);
                                      await supabase.channel('device_logout').send({
                                          type: 'broadcast',
                                          event: 'force_logout',
                                          payload: { device_id: newDeviceAlert.deviceId, user_id: userProfile.id }
                                      });
                                      showToast('устройство отключено', 'success');
                                      setNewDeviceAlert(null);
                                  }
                              }}
                              className="flex-1 px-4 py-2.5 bg-red-500/20 border border-red-500/30 rounded-xl text-red-400 text-xs font-bold uppercase tracking-wider hover:bg-red-500/30 transition-all active:scale-95"
                          >
                              Отключить
                          </button>
                      </div>
