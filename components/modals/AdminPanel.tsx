...
import { X, LayoutDashboard, Users, Radio, Server, Activity, Search, Loader2, Ban, Crown, BadgeCheck, UserX, Skull, Send, AlertTriangle, Globe, Eye, Trash2, MousePointerClick, FileText, Download, UserMinus } from 'lucide-react';
...
export const AdminPanel: React.FC<AdminPanelProps> = ({ onClose, showToast, onlineUsers, onBroadcast, onToggleMaintenance, isMaintenanceMode, userProfile, onUpdateStatus }) => {
  const [adminTab, setAdminTab] = useState<'dashboard' | 'users' | 'broadcast' | 'system' | 'logs'>('dashboard');
  const [dashboardStats, setDashboardStats] = useState({ users: 0, messages: 0, groups: 0, onlineNow: 0 });
    ...
    try {
        if (action === 'verify') {
            ...
            await supabase.rpc('log_admin_action', { 
                action_type: value ? 'verify_user' : 'unverify_user', 
                target_id: userId 
            });
            ...
            await supabase.rpc('log_admin_action', { 
                action_type: value ? 'grant_admin' : 'revoke_admin', 
                target_id: userId 
            });
            ...
            await supabase.rpc('log_admin_action', { 
                action_type: value ? 'ban_user' : 'unban_user', 
                target_id: userId 
            });
    ...
    if (onBroadcast) {
        ...
        if (success) {
            await supabase.rpc('log_admin_action', { 
                ...
                action_details: { message: adminBroadcastMsg } 
            });
  ...
  const handlePurgeOldMessages = async () => {
      ...
      try {
          ...
          await supabase.rpc('log_admin_action', { 
              ...
              action_details: { deleted_count: count || 0 } 
          });
  ...
  const handleSetMaintenanceMode = async (enable: boolean) => {
      ...
      if (success) {
          await supabase.rpc('log_admin_action', { 
              action_type: enable ? 'enable_maintenance' : 'disable_maintenance' 
          });
  ...
  const handleSetGhostMode = async (enable: boolean) => {
      ...
      try {
          ...
          await supabase.rpc('log_admin_action', { 
              action_type: enable ? 'enable_ghost_mode' : 'disable_ghost_mode' 
          });
  ...
  const handleDeleteInactiveUsers = async () => {
      if(!confirm("Удалить пользователей, которые не заходили 90+ дней?")) return;
      setIsAdminActionLoading(true);
      try {
          ...
          const { data: inactiveUsers } = await supabase
              .from('profiles')
              .select('id')
              .lt('last_seen', ninetyDaysAgo.toISOString());
          
          if (!inactiveUsers || inactiveUsers.length === 0) {
              showToast("Нет неактивных пользователей", "info");
              setIsAdminActionLoading(false);
              return;
          }
          ...
          await supabase.from('user_devices').delete().in('user_id', userIds);
          ...
          await supabase.rpc('log_admin_action', { 
              action_type: 'delete_inactive_users', 
              action_details: { deleted_count: userIds.length } 
          });
  ...
  const handleExportStats = async () => {
      ...
      try {
          ...
          a.href = url;
          a.download = `vellor_stats_${Date.now()}.json`;
          a.click();
  ...
  return (
    <div className="flex flex-col h-full bg-[#050505]">
        ...
        {/* Tabs */}
        <div className="flex p-2 gap-2 bg-black/20 border-b border-white/5 overflow-x-auto no-scrollbar">
            ...
            <button onClick={() => setAdminTab('logs')} className={`flex-1 py-3 rounded-xl flex items-center justify-center gap-2 text-[10px] font-bold uppercase tracking-wider transition-all ${adminTab === 'logs' ? 'bg-white/10 text-white' : 'text-white/40 hover:bg-white/5'}`}><FileText size={14}/> Logs</button>
        </div>
            ...
            {adminTab === 'dashboard' && (
                <div className="space-y-6">
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="p-5 bg-white/5 border border-white/5 rounded-2xl">
                            <p className="text-[10px] text-white/40 font-bold uppercase">Groups</p>
                            <h3 className="text-2xl font-black text-blue-400 mt-1">{dashboardStats.groups}</h3>
                        </div>
                        <div className="p-5 bg-white/5 border border-white/5 rounded-2xl">
                            <p className="text-[10px] text-white/40 font-bold uppercase">Online Now</p>
                            <h3 className="text-2xl font-black text-yellow-400 mt-1">{dashboardStats.onlineNow}</h3>
                        </div>
                    </div>
            ...
            {adminTab === 'system' && (
                ...
                <div className="p-5 bg-red-900/10 border border-red-500/20 rounded-2xl">
                        ...
                        <div className="space-y-3">
                            ...
                            <button onClick={handleDeleteInactiveUsers} disabled={isAdminActionLoading} className="w-full py-3 bg-red-500/20 hover:bg-red-500/40 rounded-xl text-red-300 font-bold uppercase text-[10px] tracking-wider flex items-center justify-center gap-2 transition-all active:scale-95">
                                {isAdminActionLoading ? <Loader2 className="animate-spin" /> : <UserMinus size={14}/>} Delete Inactive Users ({'>'}90 days)
                            </button>
                        </div>
                </div>
                ...
                <div className="p-5 bg-blue-500/10 border border-blue-500/20 rounded-2xl">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <h4 className="text-xs font-bold uppercase text-white">Export Data</h4>
                        </div>
                        <button onClick={handleExportStats} disabled={isAdminActionLoading} className="w-full py-3 bg-blue-500/20 hover:bg-blue-500/40 rounded-xl text-blue-300 font-bold uppercase text-[10px] tracking-wider flex items-center justify-center gap-2 transition-all active:scale-95">
                            {isAdminActionLoading ? <Loader2 className="animate-spin" /> : <Download size={14}/>} Export Statistics
                        </button>
                </div>
                </div>
            )}
                ...
                <div className="space-y-3">
                    <div className="p-4 bg-white/5 border border-white/5 rounded-2xl">
                        <div className="flex items-center gap-3 mb-2">
                            ...
                            <h4 className="text-xs font-bold uppercase text-white">Admin Activity Logs</h4>
                        </div>
                        <p className="text-[10px] text-white/40">Last 50 admin actions</p>
                    </div>
                    {adminLogs.length === 0 && (
                        <div className="p-8 text-center text-white/30 text-xs">No logs yet</div>
                    )}
                    {adminLogs.map((log, idx) => {
                        const actionText = log.action_type ? formatActionType(log.action_type) : 'Unknown Action';
                        return (
                            <div key={idx} className="p-4 bg-white/5 border border-white/5 rounded-xl hover:bg-white/10 transition-all">
                                <div className="flex items-start justify-between gap-3">
                                    <div className="flex-1">
                                        <p className="text-xs font-bold text-white mb-1">{actionText}</p>
                                        {log.target_id && <p className="text-[10px] text-white/40">Target: {log.target_id.slice(0, 8)}...</p>}
                                        {log.action_details && <p className="text-[10px] text-white/50 mt-1">{JSON.stringify(log.action_details)}</p>}
                                        {log.admin_id && <p className="text-[10px] text-white/30 mt-1">Admin: {log.admin_id.slice(0, 8)}...</p>}
                                    </div>
                                    <div className="text-right">
                                        <p className="text-[9px] text-white/30">{log.created_at ? new Date(log.created_at).toLocaleDateString() : 'N/A'}</p>
                                        <p className="text-[9px] text-white/30">{log.created_at ? new Date(log.created_at).toLocaleTimeString() : 'N/A'}</p>
                                    </div>
                                </div>
                            </div>
                        );
