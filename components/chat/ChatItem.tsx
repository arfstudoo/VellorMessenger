...
import { Pin, BellOff, Check, CheckCheck, PenLine, Mic, MapPin, Crown, BadgeCheck, Edit3 } from 'lucide-react';
...
export const ChatItem: React.FC<ChatItemProps> = memo(({ chat, activeChatId, onSelectChat, onContextMenu, onlineUsers, typingUsers, settings, myPrivacyLastSeen }) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative shrink-0">
          <div className="w-[52px] h-[52px] rounded-[1.2rem] border-2 border-white/10 overflow-hidden glass-panel shadow-lg group-hover:border-white/20 transition-all">
            {isAvatarHidden ? (
                <div className="w-full h-full bg-gradient-to-br from-gray-900 to-gray-800"/>
            ) : (
                <LazyImage 
                  src={chat.user.avatar || 'https://via.placeholder.com/56'} 
                  alt={chat.user.name}
          ...
          {!chat.user.isGroup && !isStatusHidden && (
              <div className="absolute -bottom-1 -right-1 ring-2 ring-black rounded-full">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center justify-between">
            ...
            {chat.lastMessage?.senderId === 'me' && typers.length === 0 && !chatHasDraft && (
  ...
  return (
    ...
    prevProps.chat.lastMessage?.isRead === nextProps.chat.lastMessage?.isRead &&
    prevProps.activeChatId === nextProps.activeChatId &&
    prevProps.onlineUsers.get(nextProps.chat.user.id) === nextProps.onlineUsers.get(nextProps.chat.user.id) &&
    (prevProps.typingUsers[nextProps.chat.id]?.length || 0) === (nextProps.typingUsers[nextProps.chat.id]?.length || 0)
  );
});
