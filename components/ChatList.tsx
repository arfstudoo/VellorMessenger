...
import { Menu, X, Settings, User, LogOut, ChevronLeft, Crown, BadgeCheck, History, ShieldAlert, Check, Folder, Users, MessageCircle, Monitor, Keyboard, Zap, Bug, Sparkles, Phone, ArrowUpRight, ArrowDownLeft, Clock, AlertCircle, Eye, EyeOff, Lock, Smartphone, CloudLightning, Calendar, Save, AtSign, Search, Forward, Gem } from 'lucide-react';
...
export const ChatList: React.FC<ChatListProps> = ({ 
  ...
  const [activeModal, setActiveModal] = useState<'profile' | 'settings' | 'privacy' | 'privacy_item' | 'new_chat' | 'create_group' | 'nft' | 'admin_login' | 'admin_panel' | 'changelog' | 'calls' | 'devices' | null>(null);
  ...
  useEffect(() => {
      ...
      if (lastVersion !== '2.4.4') {
          setShowChangelogAlert(true);
      }
  }, []);

  const handleOpenChangelog = () => {
      localStorage.setItem('vellor_version', '2.4.4');
  ...
  const handleAdminLogin = async () => {
      if (!adminPin.trim()) {
          showToast("Введите PIN", "warning");
          return;
      }
      
      try {
          // Вызываем безопасную функцию на backend
          const { data, error } = await supabase.rpc('verify_admin_pin', { input_pin: adminPin });
          
          if (error) {
              console.error('Admin verification error:', error);
              showToast("Ошибка проверки PIN", "error");
              setAdminPin("");
              return;
          }
          
          if (data === true) {
              // PIN верный, обновляем профиль
        ...
        <AnimatePresence>
            {showChangelogAlert && (
                <MDiv initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                    <button onClick={handleOpenChangelog} className="w-full bg-gradient-to-r from-vellor-red/20 to-transparent border border-vellor-red/30 rounded-xl p-2.5 flex items-center gap-3 mb-2 group hover:from-vellor-red/30 transition-all">
                        <div className="bg-vellor-red rounded-lg p-1 text-white animate-pulse"><Sparkles size={14}/></div>
                        <div className="text-left flex-1">
                            <p className="text-[10px] font-bold text-vellor-red uppercase tracking-wider">Обновление v3.0.0</p>
                            <p className="text-[10px] text-white/70">черновики, пересылка, упоминания</p>
                        </div>
                    </button>
                </MDiv>
            )}
      ...
      <AnimatePresence>
          ...
          <>
            ...
            <MDiv initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -20, opacity: 0 }} className="absolute top-20 left-4 w-60 bg-[#050505] border border-white/10 rounded-3xl z-50 p-2 shadow-2xl max-h-[80vh] overflow-y-auto">
              ...
              <button onClick={async () => {
                // Получаем device_id перед удалением
                const deviceId = localStorage.getItem('vellor_device_id');
                
                // Удаляем устройство из базы
                if (deviceId && userProfile.id) {
                  await supabase.from('user_devices').delete().eq('device_id', deviceId).eq('user_id', userProfile.id);
                }
                
                // Выходим и очищаем все
                await (supabase.auth as any).signOut();
                localStorage.clear();
                sessionStorage.clear();
                window.location.href = '/';
              }} className="flex items-center gap-4 w-full p-4 text-red-500/80 hover:text-red-500 rounded-2xl text-xs font-black tracking-widest uppercase transition-all active:scale-95"><LogOut size={16}/> Выйти</button>
            </MDiv>
          </>
        )}
        ...
        {activeModal && (
          <MDiv initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.3 }} className="absolute inset-0 bg-[#0a0a0a] z-[60] flex flex-col overflow-hidden">
             ...
             {activeModal === 'admin_panel' && userProfile.isAdmin && (
             ...
             {activeModal === 'settings' && (
                 <SettingsModal onClose={() => setActiveModal(null)} userProfile={userProfile} onUpdateStatus={onUpdateStatus} onSetTheme={onSetTheme} currentThemeId={currentThemeId} settings={settings} onUpdateSettings={onUpdateSettings} playPreviewSound={playPreviewSound} playingSoundId={playingSoundId} onOpenPrivacy={() => setActiveModal('privacy')} onOpenChangelog={() => setActiveModal('changelog')} onOpenProfile={() => setActiveModal('profile')} onOpenDevices={() => setActiveModal('devices')} />
             )}
             ...
             {activeModal === 'devices' && <DevicesModal onClose={() => setActiveModal(null)} userId={userProfile.id} showToast={showToast} />}
             ...
             {activeModal === 'changelog' && (
                 <div className="flex flex-col h-full bg-[#050505] relative">
                     <div className="p-6 border-b border-white/5 flex items-center justify-between bg-black/40 backdrop-blur-xl sticky top-0 z-10 shrink-0">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-[9px] text-white/40 font-mono">Build v3.0.0</p>
                            </div>
                        </div>
                     ...
                     <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar pb-24">
                         ...
                         {/* VERSION 3.0.0 (NEW) - MASSIVE UPDATE */}
                         <div className="relative pl-6 border-l-2 border-vellor-red space-y-4">
                             <div className="absolute -left-[7px] top-0 w-3 h-3 rounded-full bg-vellor-red shadow-[0_0_15px_#ff0033] animate-pulse" />
                             <div>
                                 <h3 className="text-2xl font-black text-white uppercase tracking-tight">massive update</h3>
                                 <p className="text-[10px] text-white/40 font-bold uppercase tracking-widest mt-1">v3.0.0 • 27.02.2026</p>
                             </div>
                             
                             <div className="space-y-3">
                                 {/* ADMIN PANEL */}
                                 <div className="p-4 bg-gradient-to-br from-yellow-500/10 to-transparent border border-yellow-500/20 rounded-2xl relative overflow-hidden group">
                                     <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/5 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-1000" />
                                     <h4 className="text-xs font-bold text-white mb-2 flex items-center gap-2"><Crown size={14} className="text-yellow-400"/> админка расширена</h4>
                                     <p className="text-sm text-white/80 leading-relaxed mb-2">
                                         добавлен таб logs для просмотра всех админских действий. удаление неактивных пользователей (90+ дней). экспорт статистики в json. расширенный dashboard с группами и онлайн счетчиком
                                     </p>
                                     <div className="flex flex-wrap gap-1.5">
                                         <span className="text-[9px] px-2 py-0.5 bg-white/10 rounded-full text-white/60">logs</span>
                                         <span className="text-[9px] px-2 py-0.5 bg-white/10 rounded-full text-white/60">cleanup</span>
                                         <span className="text-[9px] px-2 py-0.5 bg-white/10 rounded-full text-white/60">export</span>
                                         <span className="text-[9px] px-2 py-0.5 bg-white/10 rounded-full text-white/60">stats</span>
                                     </div>
                                 </div>
                                 
                                 {/* NFT GALLERY */}
                                 <div className="p-4 bg-gradient-to-br from-fuchsia-500/10 to-transparent border border-fuchsia-500/20 rounded-2xl relative overflow-hidden group">
                                     <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/5 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-1000" />
                                     <h4 className="text-xs font-bold text-white mb-2 flex items-center gap-2"><Gem size={14} className="text-fuchsia-400"/> nft галерея</h4>
                                     <p className="text-sm text-white/80 leading-relaxed mb-2">
                                         обновленный дизайн коллекции nft с анимированными частицами, плавными градиентами и улучшенной типографикой. название изменено на "origins"
                                     </p>
                                     <div className="flex flex-wrap gap-1.5">
                                         <span className="text-[9px] px-2 py-0.5 bg-white/10 rounded-full text-white/60">particles</span>
                                         <span className="text-[9px] px-2 py-0.5 bg-white/10 rounded-full text-white/60">gradients</span>
                                         <span className="text-[9px] px-2 py-0.5 bg-white/10 rounded-full text-white/60">animations</span>
                                     </div>
                                 </div>
                                 
                                 {/* DRAFTS */}
                                 <div className="p-4 bg-gradient-to-br from-orange-500/10 to-transparent border border-orange-500/20 rounded-2xl relative overflow-hidden group">
                                     <h4 className="text-xs font-bold text-white mb-2 flex items-center gap-2"><Save size={14} className="text-orange-400"/> черновики</h4>
                                     <p className="text-sm text-white/80 leading-relaxed mb-2">
                                         несохраненный текст автоматически сохраняется при переключении чатов. индикатор в списке чатов
                                     </p>
                                     <div className="flex flex-wrap gap-1.5">
                                         <span className="text-[9px] px-2 py-0.5 bg-white/10 rounded-full text-white/60">auto-save</span>
                                         <span className="text-[9px] px-2 py-0.5 bg-white/10 rounded-full text-white/60">localStorage</span>
                                         <span className="text-[9px] px-2 py-0.5 bg-white/10 rounded-full text-white/60">500ms debounce</span>
                                     </div>
                                 </div>
                                 
                                 {/* MENTIONS */}
                                 <div className="p-4 bg-gradient-to-br from-blue-500/10 to-transparent border border-blue-500/20 rounded-2xl relative overflow-hidden group">
                                     <h4 className="text-xs font-bold text-white mb-2 flex items-center gap-2"><AtSign size={14} className="text-blue-400"/> упоминания</h4>
                                     <p className="text-sm text-white/80 leading-relaxed mb-2">
                                         упоминай участников в группах через @. автоподсказки с поиском, подсветка в сообщениях. кликабельные упоминания открывают профиль
                                     </p>
                                     <div className="flex flex-wrap gap-1.5">
                                         <span className="text-[9px] px-2 py-0.5 bg-white/10 rounded-full text-white/60">@username</span>
                                         <span className="text-[9px] px-2 py-0.5 bg-white/10 rounded-full text-white/60">clickable</span>
                                         <span className="text-[9px] px-2 py-0.5 bg-white/10 rounded-full text-white/60">profiles</span>
                                     </div>
                                 </div>
                                 ...
                                 <div className="p-4 bg-gradient-to-br from-white/5 to-transparent border border-white/5 rounded-2xl relative overflow-hidden group">
                                     <h4 className="text-xs font-bold text-white mb-2 flex items-center gap-2"><Search size={14} className="text-purple-400"/> поиск в чате</h4>
                                     <p className="text-sm text-white/80 leading-relaxed">
                                         ищи сообщения по тексту внутри чата. навигация вперед/назад, автоскролл с подсветкой
                                     </p>
                                 </div>
                                 
                                 {/* FORWARD */}
                                 <div className="p-4 bg-gradient-to-br from-white/5 to-transparent border border-white/5 rounded-2xl relative overflow-hidden group">
                                     <h4 className="text-xs font-bold text-white mb-2 flex items-center gap-2"><Forward size={14} className="text-cyan-400"/> пересылка</h4>
                                     <p className="text-sm text-white/80 leading-relaxed">
                                         пересылай сообщения в другие чаты. показывает "Переслано от [имя]" как в телеграме
                                     </p>
                                 </div>
                                 
                                 {/* UI IMPROVEMENTS */}
                                 <div className="p-4 bg-gradient-to-br from-white/5 to-transparent border border-white/5 rounded-2xl relative overflow-hidden group">
                                     <h4 className="text-xs font-bold text-white mb-2 flex items-center gap-2"><Sparkles size={14} className="text-yellow-400"/> дизайн</h4>
                                     <p className="text-sm text-white/80 leading-relaxed mb-2">
                                         сообщения теперь темно-синие вместо красных. яркие галочки прочитанности. улучшенное поле ввода с градиентом
                                     </p>
                                     <div className="flex flex-wrap gap-1.5">
                                         <span className="text-[9px] px-2 py-0.5 bg-white/10 rounded-full text-white/60">blue theme</span>
                                         <span className="text-[9px] px-2 py-0.5 bg-white/10 rounded-full text-white/60">bright ticks</span>
                                         <span className="text-[9px] px-2 py-0.5 bg-white/10 rounded-full text-white/60">gradient button</span>
                                     </div>
                                 </div>
                                 
                                 {/* PERFORMANCE */}
                                 <div className="p-4 bg-gradient-to-br from-white/5 to-transparent border border-white/5 rounded-2xl relative overflow-hidden group">
                                     <h4 className="text-xs font-bold text-white mb-2 flex items-center gap-2"><Zap size={14} className="text-green-400"/> производительность</h4>
                                     <p className="text-sm text-white/80 leading-relaxed">
                                         батчинг realtime обновлений (50ms), оптимизация ре-рендеров, throttle для скролла
                                     </p>
                                 </div>
                             </div>
                         </div>
                         ...
                         <div className="relative pl-6 border-l border-white/10 space-y-4 opacity-60 hover:opacity-100 transition-opacity">
                             ...
                             <div>
                                 <h3 className="text-lg font-black text-white uppercase tracking-tight">устройства</h3>
                                 <p className="text-[10px] text-white/40 font-mono">v2.4.4</p>
                             </div>
                             <div className="space-y-2">
                                 <div className="p-3 bg-white/5 border border-white/5 rounded-xl"><p className="text-xs text-white/70">управление сеансами, автотрекинг устройств</p></div>
                                 <div className="p-3 bg-white/5 border border-white/5 rounded-xl"><p className="text-xs text-white/70">безопасное удаление, зеленый бейдж текущего</p></div>
                             </div>
                         </div>
                         ...
                         <div className="relative pl-6 border-l border-white/10 space-y-4 opacity-60 hover:opacity-100 transition-opacity">
                             <div className="absolute -left-[5px] top-0 w-2.5 h-2.5 rounded-full bg-white/20" />
                             <div>
                                 <h3 className="text-lg font-black text-white uppercase tracking-tight">оптимизация</h3>
                                 <p className="text-[10px] text-white/40 font-mono">v2.4.3</p>
                             </div>
                             <div className="space-y-2">
                                 <div className="p-3 bg-white/5 border border-white/5 rounded-xl"><p className="text-xs text-white/70">lazy loading, мемоизация, throttle для скролла</p></div>
                                 <div className="p-3 bg-white/5 border border-white/5 rounded-xl"><p className="text-xs text-white/70">батчинг realtime обновлений, gpu ускорение</p></div>
                             </div>
                         </div>
                         ...
                         <div className="relative pl-6 border-l border-white/10 space-y-4 opacity-60 hover:opacity-100 transition-opacity">
                             <div className="absolute -left-[5px] top-0 w-2.5 h-2.5 rounded-full bg-white/20" />
                             <div>
                                 <h3 className="text-lg font-black text-white uppercase tracking-tight">дизайн чатов</h3>
                                 <p className="text-[10px] text-white/40 font-mono">v2.4.2</p>
                             </div>
                             <div className="space-y-2">
                                 <div className="p-3 bg-white/5 border border-white/5 rounded-xl"><p className="text-xs text-white/70">улучшил дизайн сообщений и аватаров</p></div>
                             </div>
                         </div>
                         ...
                         <div className="relative pl-6 border-l border-white/10 space-y-4 opacity-60 hover:opacity-100 transition-opacity">
                             <div className="absolute -left-[5px] top-0 w-2.5 h-2.5 rounded-full bg-white/20" />
                             <div>
                                 <h3 className="text-lg font-black text-white uppercase tracking-tight">шрифт и настройки</h3>
                                 <p className="text-[10px] text-white/40 font-mono">v2.4.1</p>
                             </div>
                             <div className="space-y-2">
                                 <div className="p-3 bg-white/5 border border-white/5 rounded-xl"><p className="text-xs text-white/70">добавил шрифт inter и переделал настройки</p></div>
                             </div>
                         </div>
                         ...
                         <div className="relative pl-6 border-l border-white/10 space-y-4 opacity-60 hover:opacity-100 transition-opacity">
                             <div className="absolute -left-[5px] top-0 w-2.5 h-2.5 rounded-full bg-white/20" />
                             <div>
                                 <h3 className="text-lg font-black text-white uppercase tracking-tight">дизайн переделал</h3>
                                 <p className="text-[10px] text-white/40 font-mono">v2.4.0</p>
                             </div>
                             <div className="space-y-2">
                                 <div className="p-3 bg-white/5 border border-white/5 rounded-xl"><p className="text-xs text-white/70">переделал весь дизайн с glass эффектами и анимациями</p></div>
                             </div>
                         </div>
                         ...
                         <div className="relative pl-6 border-l border-white/10 space-y-4 opacity-60 hover:opacity-100 transition-opacity">
                             <div className="absolute -left-[5px] top-0 w-2.5 h-2.5 rounded-full bg-white/20" />
                             <div>
                                 <h3 className="text-lg font-black text-white uppercase tracking-tight">чат улучшил</h3>
                                 <p className="text-[10px] text-white/40 font-mono">v2.3.6</p>
                             </div>
                             <div className="space-y-3">
                                 <div className="p-4 bg-white/5 border border-white/5 rounded-2xl"><h4 className="text-xs font-bold text-white mb-2 flex items-center gap-2"><Calendar size={14} className="text-blue-400"/> разделители дат</h4><p className="text-sm text-white/80 leading-relaxed">добавил разделители дат в чатах</p></div>
                                 <div className="p-4 bg-white/5 border border-white/5 rounded-2xl"><h4 className="text-xs font-bold text-white mb-2 flex items-center gap-2"><MessageCircle size={14} className="text-green-400"/> непрочитанные</h4><p className="text-sm text-white/80 leading-relaxed">добавил полоску непрочитанных сообщений</p></div>
                             </div>
                         </div>
                     </div>
                 </div>
